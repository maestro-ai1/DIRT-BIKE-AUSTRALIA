...
// ─── Module-scope helpers (stable — never recreated on CartDrawer re-render) ──
...
function Field({
  ...
  return (
    <div>
      ...
      {children}
      {error && (
        <p id={`${id}-error`} role="alert" className="text-[10px] text-red-500 mt-1 font-medium">
          {error}
        </p>
      )}
    </div>
...
// ─── CheckoutForm — module scope so it never remounts on CartDrawer updates ──
...
function CheckoutForm({
  ...
  const validate = (): Record<string, string> => {
    ...
    if (!form.suburb.trim())   errs.suburb   = 'Suburb / City is required';
  ...
  const handleWhatsApp = (e: React.MouseEvent) => {
    ...
    const fullSuburbState = [form.suburb, form.state, form.postcode].filter(Boolean).join(' ') || 'Australia';
    const fullAddress    = [form.address, fullSuburbState].filter(Boolean).join(', ');
    ...
    const waUrl = waOrderLink({
      ...
      total: currentTotal,
      paymentMethod:
        paymentMethod === 'crypto'        ? 'Crypto (-10%)'  :
        paymentMethod === 'payid'         ? 'PayID'          : 'Bank Transfer',
      customerName:  form.name  || 'Website Customer',
    ...
    // Non-blocking save so admin portal can see it
    fetch('/api/order/', {
      method: 'POST',
      ...
      body: JSON.stringify({
        ...
        channel: 'whatsapp',
        customerName: form.name  || 'Website Customer',
        email:  form.email || `wa-${form.phone.replace(/\D/g, '')}@customer.local`,
        phone:  form.phone || 'Via WhatsApp',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const orderRef = generateOrderRef();
    const fullSuburbState = [form.suburb, form.state, form.postcode].filter(Boolean).join(' ');
    const fullAddress     = [form.address, fullSuburbState].filter(Boolean).join(', ');
    ...
    try {
      ...
      if (res.ok) {
        ...
        setIsCartOpen(false);
        router.push(
          `/thank-you-order/?ref=${orderRef}&email=${encodeURIComponent(form.email)}&method=${paymentMethod}&total=${currentTotal}`
        );
      } else {
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} noValidate className="p-4 space-y-3">
      ...
      <div className="border-t border-slate-100 pt-3 space-y-3">
        ...
        <div className="grid grid-cols-6 gap-2">
          <div className="col-span-3">
            <Field id="suburb" label="Suburb / City" required error={errors.suburb}>
              <input
                id="suburb"
                type="text"
...
// ─── CartDrawer (shell only — no form state here) ────────────────────────────
  ...
  return (
    ...
    >
      <div
        ...
        onClick={() => checkoutMode === 'cart' && setIsCartOpen(false)}
      />
              ...
              <div className="bg-orange-50 border border-orange-200/80 rounded-xl p-3.5 flex items-start gap-2.5">
                ...
                <div className="text-xs text-orange-950">
                  ...
                  <div className="text-orange-800 text-[11px] mt-0.5">
                    Pay with Bitcoin, USDT, or ETH to deduct{' '}
                    <strong>${cryptoDiscountAmount.toLocaleString()} AUD</strong>.
                  </div>
                </div>
              </div>
            </div>
          ) : (
            /* ── Checkout form (stable module-scope component) ── */
            <CheckoutForm
              items={items}
              subtotal={subtotal}
              cryptoDiscountAmount={cryptoDiscountAmount}
              totalWithCrypto={totalWithCrypto}
              standardTotal={standardTotal}
              shippingFee={shippingFee}
              isFreeShipping={isFreeShipping}
              clearCart={clearCart}
              setIsCartOpen={setIsCartOpen}
              onBack={() => setCheckoutMode('cart')}
            />
          )}
        </div>
